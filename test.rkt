...
(define case-2 "(let ((x 5) (y 7) (z 9))
                       (let ((y x) (x y))
                         (let ((z (- z y)))
                           (+ x y z))))")

(define case-3 "(let* ([a 3] [b (* 2 a)]) (cons a b))")
...
(define case-16 "((lambda x (cdr x)) 1 2 3)")
(define case-17 "(list 1 2 3 (list 4 5) 6)")
(define case-18 "((lambda a (cons 5 a)) 0 1 2 3 4)")
(define case-19 "(apply + (list 1 3 4))")
(define case-20 "(apply (lambda x (cdr x)) (list 1 3 4))")
(define case-21 "(apply (lambda (a b c) b) (list 1 (list 5 6) 4))")
(define case-22 "(((call/cc (λ (x) ((x x) x))) (λ (y) y)) #t)")
...
(begin
  ...
  (cek-interp (parse (open-input-string case-2))) 
